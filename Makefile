...
build-docker-prod:
	docker build -t mattgleich/contrihat:latest .
build-docker-dev:
	docker build -f dev.Dockerfile -t mattgleich/contrihat:test .
build-docker-dev-lint:
	docker build -f dev.lint.Dockerfile -t mattgleich/contrihat:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm contrihat
