...
build-docker-prod:
	docker build -f docker/Dockerfile -t mattgleich/contrihat:latest .
build-docker-dev:
	docker build -f docker/dev.Dockerfile -t mattgleich/contrihat:test .
build-docker-dev-lint:
	docker build -f docker/dev.lint.Dockerfile -t mattgleich/contrihat:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm contrihat
...
lint-in-docker: build-docker-dev-lint
	docker run mattgleich/contrihat:lint
...
test-in-docker: build-docker-dev
	docker run mattgleich/contrihat:test
